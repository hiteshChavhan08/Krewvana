...
export async function POST(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        if (!validation.success) {
            return NextResponse.json({ message: 'Invalid challenge ID format', errors: validation.error.flatten().fieldErrors }, { status: 400 });
        }
        ...
        if (now < challenge.startDate || now > challenge.endDate) {
            return NextResponse.json({ message: 'Challenge is not currently active for joining' }, { status: 400 });
        }
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            ...
            if (error.code === 'P2002' && error.meta?.target?.includes('userId') && error.meta?.target?.includes('wellnessChallengeId')) {
                 return NextResponse.json({ message: 'You are already participating in this challenge' }, { status: 409 }); // Conflict
            }
            // Handle foreign key constraint fail if challenge check was skipped
            if (error.code === 'P2003' && error.meta?.field_name?.includes('wellnessChallengeId')) {
                 return NextResponse.json({ message: 'Challenge not found.' }, { status: 404 });
            }
        }
         if (error instanceof z.ZodError) { // Handles param validation
             return NextResponse.json({ message: 'Invalid challenge ID format', errors: error.flatten().fieldErrors }, { status: 400 });
         }
         if (!await getAuthenticatedUserId()) {
             return NextResponse.json({ message: 'Unauthorized' }, { status: 401 });
        }
        return NextResponse.json({ message: 'An error occurred joining the challenge' }, { status: 500 });
    }
}