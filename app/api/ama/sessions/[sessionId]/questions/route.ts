...
export async function GET(
  ...
  try {
    ...
    whereClause.answerText = { not: null }; // Check if answerText exists

    if (answered === 'true') { whereClause.answerText = { not: null }; }
    if (answered === 'false') { whereClause.answerText = null; }

    if (approved === 'true') { whereClause.isApproved = true; }
...
export async function POST(
  ...
  try {
    ...
    if (session.status !== AMASessionStatus.UPCOMING && session.status !== AMASessionStatus.LIVE) {
        return NextResponse.json({ message: `Cannot submit questions for a session that is ${session.status.toLowerCase()}` }, { status: 400 });
    }
    ...
    const newQuestion = await prisma.aMAQuestion.create({
      data: {
       ...
       select: {
           id: true, text: true, isAnonymous: true, createdAt: true,
           submittedBy: { select: { id: true, name: true, image: true } }
       }
    });
    ...
    console.error(`Error submitting question for AMA session ${params.sessionId}:`, error);
    return new NextResponse('Internal Server Error', { status: 500 });
  }
}