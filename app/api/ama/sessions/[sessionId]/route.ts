  ...
  try {
    const [session, currentUser] = await Promise.all([
      prisma.aMASession.findUnique({
        where: { id: sessionId },
        select: { hostId: true }, // Need the hostId
      }),
      prisma.user.findUnique({
        // Need the current user's role
        where: { id: userId },
        select: { role: true },
      }),
    ]);
    if (!session || !currentUser) return false; // Session or user not found
...
export async function PUT(
  ...
  try {
    ...
    if (!canManage) {
      return new NextResponse("Forbidden: You cannot manage this session", {
        status: 403,
      });
    }
    // --- End Authorization ---
    ...
    if (
      currentSession.status === AMASessionStatus.UPCOMING &&
      newStatus !== AMASessionStatus.LIVE &&
      newStatus !== AMASessionStatus.CANCELLED
    ) {
      return NextResponse.json(
        {
          message: `Cannot transition from ${currentSession.status} to ${newStatus}`,
        },
        { status: 400 }
      );
    }
    if (
      currentSession.status === AMASessionStatus.LIVE &&
      newStatus !== AMASessionStatus.ENDED &&
      newStatus !== AMASessionStatus.CANCELLED
    ) {
      return NextResponse.json(
        {
    ...
    if (
      (currentSession.status === AMASessionStatus.ENDED ||
        currentSession.status === AMASessionStatus.CANCELLED) &&
      newStatus !== currentSession.status
    ) {
      return NextResponse.json(
        {
          message: `Session is already ${currentSession.status.toLowerCase()} and cannot be changed.`,
        },
        { status: 400 }
      );
    }
    ...
    const updatedSession = await prisma.aMASession.update({
      ...
      include: {
        // Return updated host/count info if needed
        host: { select: { id: true, name: true, image: true } },
    ...
    console.error(
      `Error updating AMA session ${params.sessionId} status:`,
      error
    );
    return new NextResponse("Internal Server Error", { status: 500 });
  }
