...
async function canModerateQuestion(userId: string, questionId: string): Promise<boolean> {
    try {
    const question = await prisma.aMAQuestion.findUnique({
        where: { id: questionId },
        select: { session: { select: { hostId: true } } }
    });
    const isAdmin = false; // Replace with actual admin check based on user role/data
    return question?.session?.hostId === userId || isAdmin;
} catch (error){
...
export async function PUT(
  ...
  try {
    ...
    if (!canModerate) {
        return new NextResponse('Forbidden: You cannot moderate this question', { status: 403 });
    }
    ...
    if (Object.keys(updateData).length === 0) {
         return NextResponse.json({ message: 'No update fields provided (isApproved or answerText)' }, { status: 400 });
    }

    const updatedQuestion = await prisma.aMAQuestion.update({
      where: { id: questionId },
      data: updateData,
      include: { // Include data needed for UI update
     ...
     const responseData = {
        ...updatedQuestion,
        submittedBy: updatedQuestion.isAnonymous ? null : updatedQuestion.submittedBy,
        submittedById: updatedQuestion.isAnonymous ? null : updatedQuestion.submittedById,
    };
  ...
  { params }: { params: { questionId: string } }
   ...
   try {
       const user = await getCurrentUser();
        ...
        const canModerate = await canModerateQuestion(user.id, questionId);
        if (!canModerate) {
            return new NextResponse('Forbidden: You cannot delete this question', { status: 403 });
        }
