  ...
  try {
    ...
    if (!access.allowed) {
      ...
      const message = userId
        ? "Forbidden: You are not a member of this private group"
    ...
    return NextResponse.json(
      { message: "An error occurred listing posts" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!access.isMember) {
      ...
      console.warn(
        `User ${userId} attempted to post in group ${groupId} without membership.`
      );
      return NextResponse.json(
        { message: "Forbidden: You must be a member to post in this group" },
        { status: 403 }
      );
    }
    ...
    const newPost = await prisma.post.create({
      ...
      include: {
        ...
        _count: {
          // Include initial comment count (0)
          select: { comments: true },
        },
      },
    });
