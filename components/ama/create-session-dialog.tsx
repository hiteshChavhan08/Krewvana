      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMsg =
            ...
            (errorData.errors // Handle Zod validation errors from API
              ? Object.entries(errorData.errors)
                  .map(
                    ([field, messages]: [string, any]) =>
                      `${field}: ${
                        messages?._errors?.join(", ") ?? "Invalid input"
                      }`
                  )
                  .join("; ")
              : errorMsg);
        } catch (jsonError) {
          // If parsing error JSON fails, try reading as text
          try {
            errorMsg = await response.text(); // Get plain text error like "Forbidden"
          } catch (textError) {
            // Keep the original status text if text reading fails
            errorMsg = response.statusText || errorMsg;
          }
        }
        // Throw the extracted/constructed error message
